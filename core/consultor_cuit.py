...
from core.generar_ta import generar_ta
...
session = Session()
session.verify = True
ZEEP_TRANSPORT = Transport(session=session, timeout=20)
...
def consultar_cuit(cuit: str) -> dict:
    ...
    if not cuit_norm.isdigit() or len(cuit_norm) != 11:
        return {"CUIT": cuit_norm, "Error": "CUIT inválido (debe tener 11 dígitos)"}
    ...
    try:
        ...
        return {"CUIT": cuit_norm, "Error": f"No se pudo autenticar con AFIP (WSAA): {e}"}

    try:
        client = Client(WSDL_PADRON, transport=ZEEP_TRANSPORT)
        respuesta = client.service.getPersonaList_v2(
            token, sign, CUIT_EMISOR, [cuit_norm]
        )
    except Exception as e:
        return {"CUIT": cuit_norm, "Error": f"AFIP no responde o error de red: {e}"}

    personas = getattr(respuesta, "persona", None)
    if not personas:
        return {"CUIT": cuit_norm, "Error": "Sin resultados en AFIP"}

    persona = personas[0]
    datos = getattr(persona, "datosGenerales", None)
    if not datos:
        return {"CUIT": cuit_norm, "Error": "Sin datos generales"}
    ...
    actividades = []
    if hasattr(persona, "datosRegimenGeneral") and hasattr(persona.datosRegimenGeneral, "actividad"):
    ...
    return {
        "CUIT": cuit_norm,
        "Razon Social o Nombre": razon_social or nombre_completo or "",
