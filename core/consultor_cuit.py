...
def consultar_cuit(cuit: str) -> dict:
    ...
    try:
        ...
        respuesta = client.service.getPersonaList_v2(token, sign, CUIT_EMISOR, [cuit_norm])
    except Exception as e:
        return {"CUIT": cuit_norm, "Error": f"Fallo consultando AFIP: {e}"}

    personas = getattr(respuesta, "persona", None)
    if not personas:
        return {"CUIT": cuit_norm, "Error": "Sin resultados"}

    persona = personas[0]
    datos = getattr(persona, "datosGenerales", None)
    if not datos:
        return {"CUIT": cuit_norm, "Error": "Sin datosGenerales"}
